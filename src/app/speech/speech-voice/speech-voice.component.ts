...
export class SpeechVoiceComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.voices$ = fromEvent(speechSynthesis, 'voiceschanged').pipe(
            ...
            tap((voices) => this.speechService2.setVoices(voices)),
        );

        this.subscription.add(
            fromEvent(this.voiceDropdown.nativeElement, 'change')
                .pipe(tap(() => this.speechService2.updateVoice(this.voiceDropdown.nativeElement.value)))
                .subscribe(),
        );
        ...
        this.subscription.add(
            merge(
                ...
                .pipe(
                    ...
                    tap((property) => this.speechService2.updateSpeech(property)),
                )
                .subscribe(),
        );
    }
